...
function ADDClickHandler() {
    ...
    if (getCookie().cartInfo != "" && getCookie().cartInfo != "[]") {
        cartArr = typeof getCookie().cartInfo == 'string' ? JSON.parse(getCookie().cartInfo) : getCookie().cartInfo;
        console.log(cartArr)
    }
    isBuy(cartArr, progoos);
    setCookie("cartInfo", JSON.stringify(cartArr), new Date(2019, 10, 16));
}
// 判断是否已经添加过购物车
